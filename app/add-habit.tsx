...
import {
  ...
  Plus, Calendar, Clock, Check, Music, Camera, Coffee, Sun, Star,
...
export default function AddHabitScreen() {
  ...
  const handleCreate = async () => {
    ...
    const habitId = await addHabit(db, {
      title: title.trim(),
      icon: selectedIcon,
      color: selectedColor,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <Text style={{ fontSize: 22, fontWeight: '700', color: colors.text }}>{t('newHabit')}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: colors.surface + 'CC', borderTopWidth: 1, borderTopColor: colors.border, padding: 20 }}>
        ...
        >
          ...
          <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: '700' }}>{t('create')}</Text>
        </Pressable>
      </View>
    </View>
  );
